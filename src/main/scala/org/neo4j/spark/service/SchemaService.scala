...
class SchemaService(
  ...
  private val queryReadStrategy = new Neo4jQueryReadStrategy(neo4j, options.tuning, filters)
  ...
  private def structForNode(labels: Seq[String] = options.nodeMetadata.labels) = {
    val structFields: mutable.Buffer[StructField] = (try {
      val query =
        s"""${generatePreamble(neo4j, options.tuning)}CALL apoc.meta.nodeTypeProperties($$config)
  ...
  private def structForGDS() = {
    val query =
      s"""
         |${generatePreamble(neo4j, options.tuning)}CALL gds.list() YIELD name, signature, type
