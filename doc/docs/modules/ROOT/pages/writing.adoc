...
In Neo4j for the nodes with label `MyNodeWithMapFlattend` you'll find this information stored:
...
Now could happen that you have a Spark job like
...
which leads to have duplicates keys while we flatten it, the Neo4j Spark connector informs you with a WARN message like this;
...
MyNodeWithMapFlattend {
    id: 'Foo',
    time: 1,
    `table.key.inner.key`: 'innerValue'
}
MyNodeWithMapFlattend {
    id: 'Bar',
    time: 1,
    `table.key.inner.key`: 'innerValue1'
}
----