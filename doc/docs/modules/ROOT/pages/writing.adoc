...
Now could  into problematic situations like the following one:
...
since the resulting flattened keys are duplicated, the Neo4j Spark will pick one of the associated value in a non-deterministic way and inform you with a `WARN` message like the following one:
