...
Simply set option `query.count` to an integer, e.g. `.option("query.count", 125000000)`
3) If you do not set any value to `query.count` we will fall back and generate `CALL { <your query> } RETURN count(*) AS count` for you.
