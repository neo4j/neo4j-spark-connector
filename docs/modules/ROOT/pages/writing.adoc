...
[IMPORTANT]
====
`MERGE` is only atomic if a uniqueness or key constraint exists on the properties it merges on.
Without one, concurrent partitions can create duplicates without reporting an error.
See xref:write/transactions.adoc#merge-safety[] before writing with `Overwrite` from more than one partition.
====

[#transactions]
== Transactions and failures

A write is not a single transaction.
The connector commits one transaction per batch of `batch.size` rows per partition, which means a failed write leaves the batches that were already committed in the database.

See xref:write/transactions.adoc[] for the full model, the retry options, and the recommended pattern for large imports.
