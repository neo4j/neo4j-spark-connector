...
[TIP]
====
For the current limitations of the connector, see xref:known-issues.adoc[], which is the canonical list of known issues and workarounds.
====
...
This is a known issue with Neo4j 3.x and Neo4j 4.0.
See xref:known-issues.adoc#column-order[Returned columns are not in the order specified in the query].
...
See xref:known-issues.adoc#deadlocks[Concurrent relationship writes can cause deadlocks].
...
The connector does not support `SaveMode.ErrorIfExists`, which is the default save mode.
Set the save mode explicitly to `SaveMode.Append` or `SaveMode.Overwrite`.

See xref:known-issues.adoc#error-if-exists[`ErrorIfExists` save mode is not supported].
...
Refer to xref:compatibility.adoc[] to choose the right version, or see xref:known-issues.adoc#version-mismatch[`NoClassDefFoundError` or `ClassNotFoundException` on startup].
...
This might happen when creating a new graph using the GDS library, because the query is run twice: once to infer the DataFrame schema and once to get the data.

See xref:known-issues.adoc#graph-already-exists[the known issues page] for the workaround.
...
See xref:known-issues.adoc#aura-ssl[SSL handshake errors when connecting to Aura from Databricks].
