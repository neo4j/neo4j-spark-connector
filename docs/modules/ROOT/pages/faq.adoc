...
Do *not* rerun a failed write unless the write is idempotent.
See xref:write/transactions.adoc#partial-failures[].

=== My write used MERGE and succeeded, but the graph has duplicate nodes. Why?

This happens almost always because there is no uniqueness or key constraint on the properties being merged on.
...
No.
The connector runs the query inside an explicit transaction, and Neo4j does not allow that clause in an explicit transaction, so the write fails.
...
You can read more xref:read/schema.adoc[here], or see xref:known-issues.adoc#mixed-types[Cast errors when a property has different types across nodes].

If you are getting this error while trying to write to Neo4j, be aware that the current version of the connector doesn't support `*SaveMode.ErrorIfExists*`, and that is the default save mode.
Change the save mode to one of `SaveMode.Append` or `SaveMode.Overwrite`.
