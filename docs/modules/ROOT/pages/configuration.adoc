...
The connector allows you to use more than one connection within a single Spark Session.
For example, you can read data from a database and write it to another database in the same session.
...
.Send the object to the driver via transaction metadata.

[source, json, role=nocollapse]
include::example$transaction-metadata-single.json[]

You can create nested structures by assigning more options.
