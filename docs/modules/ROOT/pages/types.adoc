...
|`LocalDateTime`
|`timestamp`
|label:deprecated[Deprecated since 5.4.0] footnote:[Enable legacy mode by setting option `data.conversion` to `legacy`] Set `data.conversion` to `legacy` to continue using it.
