...
|`LocalDateTime`
|`timestamp`
|label:deprecated[Deprecated (Legacy mode)] footnote:[Enable legacy mode by setting option `data.conversion` to `legacy`] Set `data.conversion` to `legacy` to continue using it.
...
|`Integer`
|`timestamp_ntz`
|label:deprecated[Deprecated (Legacy mode)] Set `data.conversion` to `legacy` to continue using it.
