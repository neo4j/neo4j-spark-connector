...
Added new `script.N` option
| Usee `scirpt.N` where `N` is an integer to run multiple successive pre-query scripts.

a|
Deprecated semi-colon in `script`
| Splitting multiple queries in `script` with a `;` is now deprecated.

a|
label:fixed[]

Option validation fix
| Fixed cases where neo4j spark options was sometimes not validated properly
