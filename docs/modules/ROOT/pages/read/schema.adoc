...
. If APOC is not installed, the connector uses random _n_ results (defined by the `schema.flatten.limit` option) of an additional Cypher query to infer the schema by the type of each column.
...
[NOTE]
====
The Cypher queries in this page are shown without their preamble.
The connector prepends a Cypher version to each of them (for example, `CYPHER 5`), either automatically or based on the `cypher.version` option.
See xref:performance/cypher-tuning.adoc[] for details.
====
...
[source, cypher]
----
MATCH (source:<source_labels>) <1>
MATCH (target:<target_labels>) <2>
MATCH (source)-[rel:<relationship>]->(target) <3>
RETURN rel
ORDER BY rand()
LIMIT <limit> <4>
----
...
<3> `<relationship>` is the relationship type provided by `relationship` option.
...
With the `query` option, the connector uses random _n_ results (defined by the `schema.flatten.limit` option) of the query result to infer the schema.
...
Some queries cannot take an `ORDER BY` and a `LIMIT` clause at the end, for example, when they already end with one.
If that is the case, the connector wraps the read query in a subquery instead:

[source, cypher]
----
CALL { <1>
  MATCH (n:Person) WITH n LIMIT 2 RETURN id(n) as id, n.age as age
}
RETURN *
ORDER BY rand()
LIMIT <limit> <2>
----
<1> With Cypher 25, the connector uses the `CALL () { ... }` form.
<2> `<limit>` is the value provided via `schema.flatten.limit`.
