...
|The timestamp property used for batch reading. Read more xref:#_streaming_property[here].
|_(none)_
|Yes
...
Writing a stream to a Neo4j instance is pretty easy and can be done using any of the xref:writing.adoc#examples[three writing strategies].

[TIP]
The same xref:read/schema.adoc[schema concepts] also apply here.
If you start a streaming read with an empty result set, you need to specify the schema using
the xref:read/define-schema.adoc#custom-schema[user defined schema], or the batch read fails.
...
As said, you can use any writing strategy: xref:writing.adoc#example-labels[Node], xref:writing.adoc#example-relationship[Relationship], or xref:writing.adoc#example-query[Query].
...
As for Sink mode, you can use any of the reading strategies: xref:reading.adoc#example-labels[Node], xref:reading.adoc#example-relationship[Relationship], or xref:reading.adoc#example-query[Query].
