...
GDS reads also support a single partition only.
See xref:known-issues.adoc#gds[Known issues and limitations] for details.
...
[TIP]
If you get a _A graph with name [name] already exists_ error, take a look at xref:known-issues.adoc#graph-already-exists[Known issues and limitations].
