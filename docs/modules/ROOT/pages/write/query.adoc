...
* `CALL { … } IN TRANSACTIONS` is *not* supported, in either the `query` option or the `script` options.
The connector runs your query inside an explicit transaction, and Neo4j does not allow that clause in an explicit transaction.
Use the `batch.size` option to control transaction size instead.
See xref:write/transactions.adoc#call-in-transactions[] for details.

* Your query is executed once per batch, and receives only the rows of that batch in `events`.
Each batch is committed separately, so a query that is not idempotent can duplicate data if a batch is retried.
