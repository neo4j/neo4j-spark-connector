...
The connector runs the query inside an explicit transaction, and Neo4j does not allow that clause in an explicit transaction.
...
* The query is executed once per batch, and receives only the rows of that batch in `events`.
